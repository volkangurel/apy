...
def get_permissions(bases, attrs):
    ...
    for base in bases:
        if not hasattr(base, 'permissions'):
            continue
...
class ApiModelPermissions(object):
    ...
    def update(self, read=None, update=None, create=None, delete=None):
        if read:
            self.read_permissions.update(read)
        if update:
            self.update_permissions.update(update)
        if create:
            self.create_permissions = create
        if delete:
            self.delete_permissions = delete
    ...
    def add(self, read_permissions=None, update_permissions=None,
        ...
        if create_permissions:
            self.create_permissions.update(create_permissions)
        if delete_permissions:
            self.delete_permissions.update(delete_permissions)
    ...
    def check_read_fields(self, request, fields):
        ...
        if 'superuser' in user_roles:
            return fields  # superusers have access to everything
        for k, v in fields:
            valid_roles = self.read_permissions.get(k, set()) | self.read_permissions.get('*', set())
            if 'public' in valid_roles or user_roles & valid_roles:
                _fields.append((k, v))
        return _fields
